...
function gradU(whichU::Int,
    ...
    ∇U = reshape(dlbar, (N1, N2 * N1 * N2 * p)) * kronU
    return ∇U
end
...
function tuckerreg(
    ...
    for s in 1:maxiter
        ...
        ∇U1 = gradU(1, cenorig, cenlag, G, p, U)
        U1 -= eta * ∇U1
        trackU1[s] = norm(∇U1)

        ∇U2 = gradU(2, cenorig, cenlag, G, p, U)
        U2 -= eta * ∇U2
        trackU2[s] = norm(∇U2)

        ∇U3 = gradU(3, cenorig, cenlag, G, p, U)
        U3 -= eta * ∇U3
        trackU3[s] = norm(∇U3)

        ∇U4 = gradU(4, cenorig, cenlag, G, p, U)
        U4 -= eta * ∇U4
        trackU4[s] = norm(∇U4)
